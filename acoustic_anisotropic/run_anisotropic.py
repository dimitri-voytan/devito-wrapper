...
class AnisotropicWrapper():
    ...
    def setup_model(self, src):
        ...
        epsilon_path = model_config['epsilon_path']
        delta_path = model_config['delta_path']
        theta_path = model_config['theta_path']
        ...
        delta = np.load(delta_path).astype(dtype)
        epsilon = np.load(epsilon_path).astype(dtype)
        theta = np.load(theta_path).astype(dtype)
    ...
    def anisotropic_setup(self, nav_i):
    ...
    def single_shot(self, nav_i, shot_num):
        ...
        solver, geometry = self.anisotropic_setup(nav_i)
...
if __name__ == '__main__':
    ...
    runner = AnisotropicWrapper(config)

...
class ElasticWrapper():
    ...
    def setup_model(self, src):
        ...
        return SeismicModel(space_order=space_order, vp=vp, vs=vs, 
                            b=1/rho, origin=origin, shape=shape, 
    ...
    def single_shot(self, nav_i, shot_num):
        ...
        solver, geometry = self.elastic_setup(nav_i)
    ...
    def dump_shot(self, out, geometry, shot_num):
        ...
        '''
        # Unpack output
        rec1, rec2, v, tau, summary = out
        rec1 = rec1.data
        rec2 = rec2.data

        # Resample gather to requested dt
        if self.config['solver']['resample_dt'] is not None:
            rec1 = resample_gather(self.config, geometry, rec1)
            rec2 = resample_gather(self.config, geometry, rec2)
